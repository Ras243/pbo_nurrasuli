...
public class Main {
    ...
    public void fungsi_kubus() {
        ...
        input_sisi = sc.nextDouble();

        Kubus kubus = new Kubus();
        kubus.setSisi(input_sisi);
        kubus.setLuas(6 * (kubus.getSisi() * kubus.getSisi()));
        kubus.setVolume(kubus.getSisi() * kubus.getSisi() * kubus.getSisi());
    ...
    public void fungsi_tabung() {
        ...
        input_jari = sc.nextDouble();
        System.out.print("Input tinggi tabung : ");
        input_tinggi = sc.nextDouble();

        Tabung tabung = new Tabung();
        tabung.setJari(input_jari);
        tabung.setTinggi(input_tinggi);
        tabung.setLuas((2 * Math.PI * tabung.getJari() * tabung.getJari()) + (2 * Math.PI * tabung.getJari() * tabung.getTinggi()));
        tabung.setVolume(Math.PI * tabung.getJari() * tabung.getJari() * tabung.getTinggi());
    ...
    public void fungsi_limas() {
        double input_alas;
        double input_tinggi_alas;
        double input_tinggi_limas;
        System.out.println("Menghitung Luas dan Volume Limas Segitiga");
        System.out.print("Input alas : ");
        input_alas = sc.nextDouble();
        System.out.print("Input tinggi alas limas : ");
        input_tinggi_alas = sc.nextDouble();
        System.out.print("Input tinggi limas : ");
        input_tinggi_limas = sc.nextDouble();

        Limas limas = new Limas();
        limas.setAlas(input_alas);
        limas.setTinggi_alas(input_tinggi_alas);
        limas.setTinggi_limas(input_tinggi_limas);
        limas.setLuas((limas.getAlas() * limas.getTinggi_alas() / 2) + 3 * (limas.getAlas() * limas.getTinggi_limas() / 2));
        limas.setVolume((limas.getAlas() * limas.getTinggi_alas() / 2) * limas.getTinggi_limas() / 3);
